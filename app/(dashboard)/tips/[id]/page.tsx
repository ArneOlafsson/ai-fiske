        ...
        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
            <Link href="/tips">
                <Button variant="ghost" size="sm" className="pl-0 hover:pl-2 transition-all">
                    <ArrowLeft className="w-4 h-4 mr-2" /> Tillbaka till Tips
                </Button>
            </Link>
            ...
            <section className="space-y-6">
                ...
                {isPremium ? (
                    ...
                    <Card className="p-6 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-amber-500/20 text-center">
                        <div className="flex flex-col items-center gap-2">
                            <Lock className="w-6 h-6 text-amber-500" />
                            <h4 className="font-bold text-amber-700 dark:text-amber-500">Endast för Premium-medlemmar</h4>
                ...
                <div className="space-y-4">
                            ...
                            <div className="space-y-1 flex-1">
                                ...
                                {comment.replies && comment.replies.length > 0 && (
                                    <div className="mt-3 space-y-2">
                                        {comment.replies.map(reply => (
                                            <div key={reply.id} className="p-3 bg-secondary/10 rounded-lg border-l-2 border-primary/50 relative">
                                                <div className="flex items-center gap-1.5 mb-1">
                                                    <Crown className="w-3 h-3 text-primary" />
                                                    <span className="font-semibold text-xs text-primary">{reply.authorName}</span>
                                ...
                                <div className="mt-3 pt-3 border-t border-border/10">
                                    {replyingTo === comment.id ? (
                                        <div className="flex gap-2 items-center bg-background/50 p-2 rounded-md border border-primary/20">
                                            <Input 
                                                ...
                                                autoFocus
                                            />
                                            <Button size="sm" className="h-8 px-3" onClick={() => handlePostReply(comment.id)}>Svara</Button>
                                            <Button size="sm" variant="ghost" className="h-8 px-2" onClick={() => { setReplyingTo(null); setReplyText(''); }}>Avbryt</Button>
                                        </div>
                                    ) : (
                                        <Button variant="outline" size="sm" className="h-7 px-3 text-xs font-semibold bg-primary/10 text-primary hover:bg-primary/20 border-primary/20" onClick={() => setReplyingTo(comment.id)}>
                                            Svara
                                        </Button>
                                    )}
                                </div>
                    ...
                    {comments.length === 0 && (
                        <p className="text-center text-muted-foreground italic py-8">Inga kommentarer än. Bli den första!</p>
                    )}
                </div>
            </section>
        </div>
